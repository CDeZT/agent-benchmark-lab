...
- The launcher now accepts simple `--model MODEL`, `--suite SUITE`, and `--repetitions N` options while preserving the older second positional suite argument. It sends a requested model only to adapters that support CLI selection. OpenCode is now a verified `cli` model-selection adapter: its current local CLI accepted `-m longcat/LongCat-2.0`, while `unspecified` is observed by a saved temporary JSON-session/export probe before task 1. The completion summary includes requested, parser-observed, and identity state.
...
- Added first-class `mimo` (MimoCode) adapter. It finds `mimo` on PATH or `~/.mimocode/bin/mimo`, invokes `mimo run --format json`, and records explicit JSONL token/cost/tool telemetry. The current real Mimo default JSONL does not expose a model identifier: this must remain pending instead of being filled from a requested label.
- Grok parsing now accepts its streaming JSON mode event-by-event. Do not claim live Grok model/usage telemetry without explicit emitted fields; this iteration did not obtain a successful real Grok telemetry probe.
- Restyled the full TUI into a calmer transcript: one harness/model/status header, focused current task, progress line, and real activity transcript. Pseudo-terminal QA at 80x24 confirmed a visible startup-verified model, workspace-change activity, one initial clear only, and no viewport scroll. It deliberately does not fabricate tool calls or reasoning details.
- The prior manual ANSI renderer was removed after a real terminal frame showed literal cursor-control garbage at the bottom. `rich.Live` now owns alternate-screen redraws. Its wide layout uses a task panel and a run-context panel side by side above a full activity stream; a 160x30 raw pseudo-terminal capture has no `[0B`/`[0A` leak.
- OpenCode default identity is no longer pending on this machine: the saved startup probe runs one isolated `opencode run --auto --format json`, extracts `sessionID`, then reads `opencode export <session-id>`. It observed `LongCat-2.0` at roughly `$0.006`. The real local `-m longcat/LongCat-2.0` call also succeeded, so OpenCode is now `model_selection=cli`; preflight must not retain the stale `configured_default_only` warning.
