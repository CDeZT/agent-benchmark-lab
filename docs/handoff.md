...
Phase 1 framework foundation is usable, but the benchmark is not finished. The project currently has 19 task definitions, 6 suites, 86 unittest test functions, real harness smoke support, Docker evaluator v1 code, and evidence-backed scoring rules that keep dimensions at 0 when evidence is absent.
...
```bash
PYTHONPATH=src python3 -m unittest discover -s tests -v       # 86 tests
