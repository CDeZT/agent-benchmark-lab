...
例如：同一默认模型下 Codex、Claude Code、opencode、Aider、Antigravity CLI、MimoCode、Grok 哪个更适合你的工作方式；或同一 harness 在明确验证模型身份后，DeepSeek、mimo、LongCat、GPT、Gemini 等模型哪个更强。
...
目前可用的内置 adapter：`aider`、`antigravity`、`claude-code`、`codex`、`grok`、`mimo`、`opencode`，以及用于框架测试的 `dummy`、`generic-command`。`antigravity` 对应官方 `agy` CLI，`mimo` 对应本机命令 `mimo`，并会在 PATH 缺失时自动尝试 `~/.mimocode/bin/mimo`。`list-adapters` 只显示内置 adapter 或你显式配置的本地 registry；示例文件不会假装某个 CLI 已安装。
...
它默认使用 `comprehensive-screening-v1`、当前 CLI 默认模型、`stress`、三重复，并自动完成：环境 doctor、preflight、实际 suite run、dashboard 刷新和 macOS 浏览器打开。所有 task 证据、suite report、matrix/bridge artifacts 和 `dashboard/` 都放在**当前目录**，不会污染源码目录。运行时终端会实时显示当前 task、重复次数、agent/评分阶段、已完成 attempts、耗时和基于已完成 attempts 的 ETA；首次 attempt 完成前 ETA 会诚实显示为 `calculating`。

默认模型是正常使用路径，不需要也不建议每次写模型参数。只有你刻意做“明确指定模型”的实验时，才使用这个高级覆盖：
...
兼容旧写法：`agent-benchmark claude-code hard-discrimination`。

只想确认 CLI、权限与证据保存链是否能工作，可以运行不可排名的一题 smoke；它会执行并生成报告，但会明确标为不能用于比较结论：

```bash
agent-benchmark claude-code --suite real-smoke --repetitions 1
```
...
PYTHONPATH=src python3 -m agent_benchmark.cli.main run-matrix \
  ...
  --adapters codex,claude-code,opencode,aider,antigravity,grok,mimo \
...
```bash
PYTHONPATH=src python3 -m agent_benchmark.cli.main preflight-matrix \
  --suite personal-probe --adapters codex,aider,antigravity,claude-code,opencode,grok,mimo \
  --models unspecified --repetitions 3
...
| `grok` | `grok --always-approve --output-format streaming-json --prompt-file` | `unspecified` 使用当前 CLI 默认；显式模型加 `-m`。 | 逐行读取可选 streaming JSON 遥测；没有明确模型、token、cost 或工具事件就保持 unavailable。 |
| `mimo` | `mimo run --format json --dangerously-skip-permissions` | `unspecified` 使用当前 CLI 默认；显式模型加 `-m`。 | MimoCode JSONL 的 `tokens.input`、`tokens.output` 和 `cost` 会被保存；当前真实默认输出未提供模型身份时必须保持 pending，不会从请求参数猜测。 |
| `antigravity` | `agy --dangerously-skip-permissions --print` | `unspecified` 使用 Antigravity 当前默认；显式模型加 `--model`。 | 当前 AGY 1.1.1 print 输出没有稳定的结构化模型、token、cost 或工具事件。默认模型不做付费探针，所有这些字段保持 unavailable/pending；显式请求也不是身份验证。 |

所有内置 adapter 都运行在每次复制的 task workspace，默认不在用户项目仓库内自动 commit。Antigravity 标准安装位置不在 PATH 时，adapter 会自动使用 `~/.local/bin/agy`；也可用 `AGENT_BENCH_ANTIGRAVITY_COMMAND` 覆盖。MimoCode 标准安装位置不在 PATH 时，adapter 会自动使用 `~/.mimocode/bin/mimo`；也可用 `AGENT_BENCH_MIMO_COMMAND` 覆盖。Grok、Mimo 与 Antigravity 的覆盖变量分别是 `AGENT_BENCH_GROK_COMMAND`、`AGENT_BENCH_MIMO_COMMAND` 与 `AGENT_BENCH_ANTIGRAVITY_COMMAND`。覆盖后先跑 `doctor` 和单题 smoke。
