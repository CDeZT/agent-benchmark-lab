...
每个完成的 CLI `run`、`run-suite`、`run-matrix` 及其 resume 入口都会自动重建 `<runs-dir>/dashboard/index.html`。不要把 dashboard 重建放进内部每道 task 的循环，否则完整矩阵会反复扫描历史 artifacts。仓库根目录的 `./benchmark` 是用户友好的 macOS 启动器：它默认归档到 `~/Documents/AgentBenchmarkResults`，执行 doctor/preflight/run/dashboard，最后调用 `open`。保持它只做编排，不复制 runner 或 scorer 逻辑。
