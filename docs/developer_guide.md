...
4. 在 `parsers/` 为可验证的模型/usage/tool telemetry 添加解析；无法确定就留空。MimoCode 的 JSONL 若只有 token/cost 而没有 `model`，必须保存前者且让模型身份保持 pending。
