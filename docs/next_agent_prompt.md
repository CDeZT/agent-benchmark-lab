...
- 宽交互终端默认运行彩色、居中的 alternate-screen 全屏 TUI：模型身份、当前任务、进度和 ETA 是首屏内容，runner 生命周期和已观察到的 workspace mutation 位于下方。首帧之后只能差量更新变化行，不能每次 heartbeat 清屏造成闪烁。任何完成、失败或 Ctrl-C 路径都必须恢复 cursor 与原 shell；`AGENT_BENCH_TUI=compact`、`AGENT_BENCH_TUI=full`、`AGENT_BENCH_PROGRESS=plain` 和 `TERM=dumb` 是兼容性边界。保持 `live_status.json` 的 activity/recent attempts/display/model 字段与测试同步；绝不把活动流写成未证实的 harness 工具调用或模型思考。
