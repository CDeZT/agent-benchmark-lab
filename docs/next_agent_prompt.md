...
- 宽交互终端默认运行居中的 alternate-screen 全屏 TUI：模型身份、当前任务、进度和 ETA 是首屏内容，runner 生命周期活动流位于下方。任何完成、失败或 Ctrl-C 路径都必须恢复 cursor 与原 shell；`AGENT_BENCH_TUI=compact`、`AGENT_BENCH_TUI=full`、`AGENT_BENCH_PROGRESS=plain` 和 `TERM=dumb` 是兼容性边界。保持 `live_status.json` 的 activity/recent attempts/display/model 字段与测试同步；绝不把活动流写成未证实的 harness 工具调用。
...
- `claude-code` 使用 `unspecified` 时，suite 创建后应通过 `model_probe.json` 一次性记录实际默认模型：只能使用临时目录、`--tools ''`、JSON 输出、`--no-session-persistence` 和 `$0.05` 硬上限；resume 必须复用已有 probe。probe 是模型身份元数据，不是任务能力证据，不能影响得分、重复统计或成本效率分。自定义 Claude command 或其他 adapter 没有等价 probe 时必须显示未观察到，不能猜测。
