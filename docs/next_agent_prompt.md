...
- 宽交互终端默认运行 Rich 管理的彩色、全宽 alternate-screen TUI：harness、模型身份、状态和可读任务 title 是首屏内容，宽屏当前任务/运行上下文两栏，下方是 runner 生命周期和已观察到的 workspace mutation。禁止恢复项目手写 cursor move/clear ANSI 或每次 heartbeat 全屏清空；Rich 必须负责 redraw 与恢复，80x24 无滚屏、无字面控制序列泄漏是回归下限。任何完成、失败或 Ctrl-C 路径都必须恢复原 shell；`AGENT_BENCH_TUI=compact`、`AGENT_BENCH_TUI=full`、`AGENT_BENCH_PROGRESS=plain` 和 `TERM=dumb` 是兼容性边界。保持 `live_status.json` 的 activity/recent attempts/display/model 字段与测试同步；绝不把活动流写成未证实的 harness 工具调用或模型思考。
- 用户友好启动器的主路径始终是 `agent-benchmark claude-code`，默认使用当前 CLI 默认模型。`--model MODEL --suite SUITE --repetitions N` 是高级覆盖选项，不能在 README、help 或正常提示中取代默认路径；也不得把它改回必须记忆环境变量的流程。OpenCode 现为 `cli` model-selection adapter，显式 `--model` 应传为 `-m provider/model`，默认模式必须用保存的 export probe 显示 observed model。
- `real-smoke` 等没有 `comparative_candidate` 的诊断 suite 仍必须能执行，用于确认 harness、权限、证据和报告链；preflight 只能将它标为不可排名 warning，不能阻止运行或把 smoke 结果混入选择结论。
- TUI 中的模型必须同时保留 requested、adapter invocation 与 parser-derived observed 事实；没有 observed model 时只显示当前 CLI default/awaiting evidence，绝不能为了视觉完整性把 canonical label、registry 或旧配置写成已验证身份。不同 attempts 观测到多个模型时保留 `observed_models` 并标记 `observed_multiple`，不可只显示最后一个。
- `claude-code` 使用 `unspecified` 时，suite 创建后应通过 `model_probe.json` 一次性记录实际默认模型：只能使用临时目录、`--tools ''`、JSON 输出、`--no-session-persistence` 和 `$0.05` 硬上限；resume 必须复用已有 probe。probe 是模型身份元数据，不是任务能力证据，不能影响得分、重复统计或成本效率分。Codex 的本地 TOML `model` 可以显示为 configured-but-unverified hint，不能进入 `detected_models` 或分数；自定义 Claude command 或其他 adapter 没有等价 probe 时必须显示 pending，不能猜测。
