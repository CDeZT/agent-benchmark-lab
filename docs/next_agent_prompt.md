...
- 宽交互终端默认运行彩色、居中的 alternate-screen 全屏 TUI：harness、模型身份、状态和可读任务 title 是首屏内容，task id/repetition、进度和 ETA 其次，runner 生命周期和已观察到的 workspace mutation 位于下方。首帧之后只能差量更新变化行，不能每次 heartbeat 清屏造成闪烁；完整 frame 和最终 cursor 必须留在实际 terminal height 内，80x24 无滚屏是回归下限。任何完成、失败或 Ctrl-C 路径都必须恢复 cursor 与原 shell；`AGENT_BENCH_TUI=compact`、`AGENT_BENCH_TUI=full`、`AGENT_BENCH_PROGRESS=plain` 和 `TERM=dumb` 是兼容性边界。保持 `live_status.json` 的 activity/recent attempts/display/model 字段与测试同步；绝不把活动流写成未证实的 harness 工具调用或模型思考。
- 用户友好启动器的主路径始终是 `agent-benchmark claude-code`，默认使用当前 CLI 默认模型。`--model MODEL --suite SUITE --repetitions N` 是高级覆盖选项，不能在 README、help 或正常提示中取代默认路径；也不得把它改回必须记忆环境变量的流程。opencode 是 `configured_default_only`，即使用户给了 `--model` 也必须显示为请求标签未被 CLI 覆盖使用。
