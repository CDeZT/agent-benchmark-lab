...
- Interactive suite/matrix runs now render a coding-agent-style alternate-screen TUI with live task/repetition progress, current phase, elapsed time, recent attempts, and an evidence-based ETA to stderr. They persist recoverable `suite-*/live_status.json` state and preserve stdout for final machine-readable output.
