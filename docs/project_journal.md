...
- Implemented Docker evaluator v1 for dependency-bound tasks: exact-version requirements generate a task image, public and hidden tests run with `--network none`, CPU/memory limits and controlled mounts, and image/build/test evidence is persisted per repetition. The host harness gets a public container-test helper so its edit-test-repair loop uses the same environment without moving login credentials into a container.
