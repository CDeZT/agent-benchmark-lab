...
- 80 unittest test functions, all expected to pass in the current tree.
...
| Evidence-backed scoring | Partial | Every non-zero score must come from saved execution evidence. Reports now distinguish verified, heuristic, and unavailable dimensions; `cost_efficiency` uses parsed token/cost only. 80 unittest tests cover framework and scoring behavior. | Replace weak trace heuristics and add browser/subagent evidence. |
