...
- 163 unittest test functions, all expected to pass in the current tree.
...
| Evidence-backed scoring | Partial | Every non-zero score must come from saved execution evidence. Reports distinguish verified, heuristic, and unavailable dimensions; workspace diffs and file-only instruction matches are heuristic rather than verified intent/tool traces; `cost_efficiency` records real telemetry but needs a predeclared task budget before producing points; model identity distinguishes verified matches, explicit unverified/mismatched requests, observed CLI defaults, and model drift across attempts. SWE-bench `error_ids` are classified as unscoreable evaluator errors rather than model failures. Historical summaries with a missing/mismatched task fingerprint are excluded from selection statistics. 163 unittest tests cover framework and scoring behavior. | Complete Docker corpus audit and replace remaining process heuristics with causal evidence. |
...
| Live terminal TUI and progress | Implemented | Wide interactive terminals use an alternate-screen coding-agent workbench: a persistent harness/model/progress rail plus a runner-derived activity stream; compact/plain/dumb terminals safely degrade. `suite-*/live_status.json` persists the same state and activity trace, while the compact launcher summary avoids a final JSON dump. | Stream safe, non-sensitive harness tool events only where an adapter exposes them; add a dashboard live-run view when a desktop UI exists. |
