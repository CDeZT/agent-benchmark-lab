...
- Task manifests.
- Suites (13 total, including foundation, calibration, personal-probe, hard/optics discrimination, a hard-to-easy selection ladder, the fixed `comprehensive-screening-v1` local+SWE cohort, and a quarantined SWE-bench metadata pilot).
- Single-task runs.
- Suite runs.
...
- 176 unittest test functions, all expected to pass in the current tree.
- Built-in Codex CLI, Aider, and Antigravity CLI adapters with conservative telemetry parsing.
...
It is not yet a finished real Claude Code versus opencode benchmark. Docker is now available through Colima, browser screenshots/pixel evidence work for local static pages, and a project-owned container task has run. `comprehensive-screening-v1` now gives a single resumable command for a fixed local cohort plus a fixed SWE-bench Verified cohort, but it intentionally reports local ten-dimension scores and official resolution rates separately. A multi-repeat real matrix and a completed official hard-ranking cohort remain unfinished. Five legacy SWE-bench records are safely marked metadata-only and rejected by the generic runner, rather than being mistaken for scored imported tasks. Model choices behind both CLIs are dynamic: the normal matrix compares their current defaults, while explicit same-model experiments remain a separate verified mode.
...
| Transparent personal decision index | Implemented | `balanced-v1` combines local verified-normalized score (55%) and official SWE resolution rate (45%), stores the full profile plus SHA-256 fingerprint, and labels missing-evidence outputs as provisional/unavailable. It does not alter local totals or radar charts. | Add a user-frozen alternative profile only when a different use case needs it. |
...
| Evidence-backed scoring | Partial | Every non-zero score must come from saved execution evidence. Reports distinguish verified, heuristic, and unavailable dimensions; workspace diffs and file-only instruction matches are heuristic rather than verified intent/tool traces; `cost_efficiency` records real telemetry but needs a predeclared task budget before producing points; model identity distinguishes verified matches, explicit unverified/mismatched requests, observed CLI defaults, saved Claude/OpenCode startup probes, configured-but-unverified defaults, and model drift across attempts. Antigravity print text is not telemetry and receives no inferred model/tool/cost credit. SWE-bench `error_ids` are classified as unscoreable evaluator errors rather than model failures. Historical summaries with a missing/mismatched task fingerprint are excluded from selection statistics. 176 unittest tests cover framework and scoring behavior. | Complete Docker corpus audit and replace remaining process heuristics with causal evidence. |
...
| Cost and duration | Partial | Duration is measured; parsed token/cost fields are carried into run summaries when harness output exposes them. Cost points require a predeclared task-level budget, avoiding an arbitrary cross-provider dollar conversion. | Define and freeze task cost budgets only for dedicated efficiency experiments; improve provider-specific usage parsing and reporting. |
...
| Real coding-agent adapters | Partial | Built-in templates and doctor checks now cover Codex, Aider, Antigravity CLI (`agy`), Claude Code, opencode, Grok, and MimoCode (`mimo`). Antigravity uses `agy --print` and supports explicit `--model`, but version 1.1.1 provides no stable structured model/tool/token/cost telemetry, so no paid default probe or inferred score is used. OpenCode uses structured JSONL and its temporary-session export probe observed the local default as `LongCat-2.0`; its local `-m longcat/LongCat-2.0` invocation was also verified. Codex JSONL tool/usage telemetry is parsed conservatively; Aider only receives model/token/cost credit when it explicitly prints them; Mimo JSONL records explicit token/cost fields but leaves a missing model identity pending. External CLI templates are runnable only when explicitly activated in a local registry, not because they appear in documentation examples. | Run fingerprinted three-repeat current-default matrices; add verified same-model experiments only where CLI selection and observed identity agree. |
...
| Doctor command | Implemented | `agent-benchmark doctor` checks local tools including `agy`, Docker daemon readiness, and adapter command env vars. | Add credential checks without exposing secrets. |
| Live terminal TUI and progress | Implemented | Wide interactive terminals use a coloured, full-width Rich alternate-screen view with task and context columns plus a real activity transcript. Rich, rather than project code, owns terminal redraw/cursor handling; pseudo-terminal QA confirms no literal cursor-control leak. The 80x24 layout remains bounded, and compact/plain/dumb terminals safely degrade. Claude and OpenCode default runs can present a saved observed startup identity before task 1; Codex config hints stay explicitly unverified, and Mimo/Antigravity stay pending when their output omits model identity. `suite-*/live_status.json` persists the same state and activity trace, while the compact launcher summary avoids a final JSON dump. | Add safe structured probes for other adapters and a dashboard live-run view when a desktop UI exists. |
| Consumer-friendly setup and native macOS app | Planned | The current `benchmark` launcher runs doctor/preflight and writes results to the chosen directory, but it still assumes a prepared CLI environment and does not repair prerequisites. | Build automatic readiness diagnosis, confirmed safe remediation, plain-language errors, and a standalone native macOS app with setup, status, run, progress, recovery, history, and dashboard. |
...
| External benchmark imports | Partial | Source-aware manifests, catalog command, validated SWE-bench Verified/Terminal-Bench Core source registry, and `preflight-authoritative` toolchain checks now exist. `comprehensive-screening-v1` executes the frozen SWE-bench hard-v2 cohort through independent official bridge repetitions and reports a separate resolution track, never a fabricated local score. `terminal-bench-bridge` plans/runs one pilot task through official `tb run` on a separate track. Legacy SWE-bench task records remain `external_frozen`, cannot be run by the generic runner, and cannot be ranked. | Run the complete hard cohort through official evaluation; execute and classify the first Terminal-Bench pilot task. |
