...
| Live terminal TUI and progress | Implemented | Wide interactive terminals use an alternate-screen coding-agent-style dashboard with progress, current task, run health, ETA, spinner, and recent attempts; compact/plain/dumb terminals safely degrade. `suite-*/live_status.json` persists the same state and the compact launcher summary avoids a final JSON dump. | Stream safe, non-sensitive harness tool events only where an adapter exposes them; add a dashboard live-run view when a desktop UI exists. |
