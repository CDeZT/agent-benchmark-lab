...
- 168 unittest test functions, all expected to pass in the current tree.
...
| Live terminal TUI and progress | Implemented | Wide interactive terminals use a coloured, centered alternate-screen agent-session view. It clears only its initial frame and differentially refreshes changed rows, preventing heartbeat flicker. Claude default runs can present an observed, low-budget startup-probe identity before task 1; Codex config hints stay explicitly unverified; runner lifecycle, workspace mutations, and later harness evidence remain distinct. Compact/plain/dumb terminals safely degrade. `suite-*/live_status.json` persists the same state and activity trace, while the compact launcher summary avoids a final JSON dump. | Add safe structured probes for other adapters and a dashboard live-run view when a desktop UI exists. |
