...
- 160 unittest test functions, all expected to pass in the current tree.
...
| Live terminal progress | Implemented | `run-suite`/resume and matrix paths show task/repetition/phase, completed attempts, elapsed time, and a completed-duration ETA on interactive stderr; `suite-*/live_status.json` persists the same state. The compact launcher summary avoids a final JSON dump. | Stream safe, non-sensitive harness tool events only where an adapter exposes them; add a dashboard live-run view when a desktop UI exists. |
