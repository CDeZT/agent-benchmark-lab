...
- 172 unittest test functions, all expected to pass in the current tree.
...
| Live terminal TUI and progress | Implemented | Wide interactive terminals use a coloured, centered alternate-screen transcript view. It clears only its initial frame and differentially refreshes changed rows, preventing heartbeat flicker. The 80x24 layout is height-budgeted so the model header cannot scroll away; it uses a readable task title plus secondary task id, lifecycle, and real workspace mutations. Claude default runs can present an observed, low-budget startup-probe identity before task 1; Codex config hints stay explicitly unverified, and Mimo stays pending when its JSONL omits a model. Compact/plain/dumb terminals safely degrade. `suite-*/live_status.json` persists the same state and activity trace, while the compact launcher summary avoids a final JSON dump. | Add safe structured probes for other adapters and a dashboard live-run view when a desktop UI exists. |
