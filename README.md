...
- 5 种内置适配器（dummy/generic-command/opencode/claude-code/grok）+ `config/harnesses.example.json` 配置化登记任意 headless CLI；示例不会自动注册为可运行 adapter，需复制为被忽略的 `config/harnesses.json` 或通过 `AGENT_BENCH_HARNESSES_FILE` 显式启用。
