"""
Simple Book Library API - Flask backend with SQLite. FIXED version.
"""
...
# FIX: update updated_at timestamp
...
def update_book(book_id):
    ...
    fields.append("updated_at = datetime('now')")  # FIX
...
# FIX: remove from reading_list first
...
def delete_book(book_id):
    ...
    if not book:
        return jsonify({"error": "Book not found"}), 404
    db.execute("DELETE FROM reading_list WHERE book_id = ?", (book_id,))  # FIX
...
def add_to_reading_list():
    ...
    db = get_db()
    book = db.execute("SELECT id FROM books WHERE id = ?", (data["book_id"],)).fetchone()  # FIX
    if not book:
        return jsonify({"error": "Book not found"}), 404
    try:
        cursor = db.execute(
...
# FIX: exclude NULL ratings from average
...
def get_stats():
    ...
    avg_rating = db.execute("SELECT AVG(rating) FROM books WHERE rating IS NOT NULL").fetchone()[0]  # FIX
