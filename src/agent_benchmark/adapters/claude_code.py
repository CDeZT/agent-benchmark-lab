...
class ClaudeCodeAdapter(ShellCommandAdapter):
    ...
    default_command_template = (
        ...
        'claude -p --dangerously-skip-permissions --no-session-persistence "$(cat {instruction_file})"; '
        "else "
        'claude -p --dangerously-skip-permissions --no-session-persistence --model "$AGENT_BENCH_MODEL" "$(cat {instruction_file})"; '
        "fi"
    )
